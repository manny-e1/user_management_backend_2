    ...
    const data = mntLogs.map((item, index: number) => {
      ...
      if (
        ...
        return {
          ...item,
          mid: index + 1,
          iRakyatStatus: item.iRakyatYN && item.approvedBy !='' && item.submissionStatus !='New' ? 'C' : '',
          iBizRakyatStatus: item.iBizRakyatYN && item.approvedBy !='' && item.submissionStatus !='New' ? 'C' : '',
        };
      } else if (
