...
import { maintenanceLogs, NewMaintenanceLog, rejectionLogs } from '@/db/schema.js';
import { logger } from '@/logger.js';
import { ERRORS } from '@/utils/errors.js';
import { desc, eq, inArray, sql, and } from 'drizzle-orm';

export async function createMntLogs(body: (Omit<NewMaintenanceLog, 'createdAt'> & {createdAt: Date})[]) {
	try {
...
export async function getMntLogs() {
	try {
		...
		const data = mntLogs.map((item, index: number) => {
			if(item.endDate.toISOString() < now) {
			...
			} else if(item.approvalStatus == "Rejected" && item.submissionStatus == "Marked") {
				return {
					...item,
					mid: index + 1,
